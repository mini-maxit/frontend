<script lang="ts">
  ...
  let sortedLeaderboard = $derived.by(() => {
    if (!resultsQuery.current?.userStats) return [];

    return [...resultsQuery.current.userStats]
      .map((userStats) => {
  ...
  let contestStats = $derived.by(() => {
    if (!sortedLeaderboard.length) return null;

    const totalParticipants = sortedLeaderboard.length;
    const totalSubmissions = sortedLeaderboard.reduce(
      (sum, user) =>
        sum + user.taskBreakdown.reduce((taskSum, task) => taskSum + task.attemptCount, 0),
      0
    );
    const totalTasksSolved = sortedLeaderboard.reduce((sum, user) => sum + user.tasksSolved, 0);
    const avgScore =
      sortedLeaderboard.reduce((sum, user) => sum + user.totalScore, 0) / totalParticipants;
