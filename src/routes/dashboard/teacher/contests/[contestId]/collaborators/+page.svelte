...
<div class="space-y-6">
  ...
  <div class="space-y-4">
    ...
    {:else if collaboratorsQuery.loading}
      <LoadingSpinner message={m.contest_collaborators_loading()} />
    {:else if collaboratorsQuery.current && collaboratorsQuery.current.length === 0}
      <EmptyState
        title={m.contest_collaborators_no_collaborators_title()}
        description={m.contest_collaborators_no_collaborators_description()}
        icon={Users}
      />
