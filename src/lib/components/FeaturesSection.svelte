<script>
	...
	const features = [
		{
			icon: Code,
			title: 'Create Algorithmic Tasks',
			description:
				"Easily design and publish programming challenges tailored to your students' skills."
		},
		{
			icon: Trophy,
			title: 'Organize Contests',
			description:
				'Host official competitions and monitor progress in real-time with built-in analytics.'
		},
		{
			icon: Users,
			title: 'Engage Students',
			description: 'Gamified experience keeps students motivated while sharpening coding skills.'
		}
	];
</script>
...
<section class="relative overflow-hidden bg-background px-4 py-16">
	...
	<div class="relative mx-auto max-w-7xl">
		...
		<div class="mb-12 text-center">
			<div
				class="mb-8 inline-block rounded-lg bg-gradient-to-r from-[var(--primary)] to-[var(--secondary)] px-8 py-3 text-primary-foreground shadow-lg"
			>
				<h2 class="m-0 text-xl tracking-tight">Why Choose MaxIT?</h2>
			</div>
		</div>
			...
			{#each features as feature, index}
				...
				>
					...
					<div class="relative z-10">
						...
						>
							<svelte:component this={feature.icon} class="h-8 w-8 text-primary-foreground" />
						</div>
