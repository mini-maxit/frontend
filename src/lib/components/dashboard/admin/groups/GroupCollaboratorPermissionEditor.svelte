    ...
    ),
    {
      ...
      async onUpdate({ form }) {
        ...
        try {
          ...
          showApiError(error, m.group_collaborators_update_error());
        }
      }
    }
  );
  ...
  function getPermissionLabel(permission: Permission): string {
    switch (permission) {
      ...
      case Permission.Owner:
        return m.group_collaborators_permission_owner();
      default:
        return permission;
    }
  }
...
{#if isEditable}
  ...
  <Popover.Root bind:open={popoverOpen}>
    <Popover.Trigger
      ...
      title={m.group_collaborators_update_title()}
    >
...
<Dialog.Root bind:open={dialogOpen}>
  <Dialog.Content>
    <Dialog.Header>
      <Dialog.Title>{m.group_collaborators_update_confirm_title()}</Dialog.Title>
      <Dialog.Description>
        {m.group_collaborators_update_confirm_description({
          userName,
