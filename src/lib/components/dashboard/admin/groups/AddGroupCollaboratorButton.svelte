    ...
    ),
    {
      ...
      async onUpdate({ form }) {
        ...
        try {
          ...
          showApiError(error, m.group_collaborators_add_error());
        }
      }
    }
  );
  ...
  function getPermissionLabel(permission: Permission): string {
    switch (permission) {
      case Permission.Edit:
        return m.group_collaborators_permission_edit();
      case Permission.Manage:
        return m.group_collaborators_permission_manage();
      default:
        return permission;
    }
  }
...
<Dialog.Root bind:open={dialogOpen} onOpenChange={(open) => !open && resetForm()}>
    ...
    <div class="relative flex flex-col items-center gap-4 text-center">
      ...
      <div>
        <h3 class="text-lg font-bold text-primary-foreground">
          {m.group_collaborators_add_title()}
        </h3>
        <p class="mt-1 text-sm text-primary-foreground/80">
          {m.group_collaborators_add_description()}
        </p>
      </div>
    </div>
  </button>
  ...
  <Dialog.Content class="max-w-2xl">
    <Dialog.Header>
      <Dialog.Title>{m.group_collaborators_add_dialog_title()}</Dialog.Title>
      <Dialog.Description>
        {m.group_collaborators_add_dialog_description()}
      </Dialog.Description>
    </Dialog.Header>
        ...
        <div class="space-y-3">
          ...
          <div class="relative">
            ...
            <Input
              ...
              type="text"
              autocomplete="off"
              placeholder={m.group_collaborators_add_user_placeholder()}
          ...
          >
            <Select.Trigger class="w-full" id="permission">
              {selectedPermission
                ...
                : m.group_collaborators_add_permission_placeholder()}
            </Select.Trigger>
            <Select.Content>
              <Select.Item value={Permission.Edit}>
                {m.group_collaborators_permission_edit()}
              </Select.Item>
              <Select.Item value={Permission.Manage}>
                {m.group_collaborators_permission_manage()}
              </Select.Item>
            </Select.Content>
          </Select.Root>
        </div>
