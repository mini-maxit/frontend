<script lang="ts">
  ...
  async function handleRemove(event: Event) {
    ...
    try {
      ...
      showApiError(error, m.group_collaborators_remove_error());
    } finally {
      isRemoving = false;
    }
  }
...
{#if canRemove}
  <Button
    ...
    title={m.group_collaborators_remove_title()}
  >
    <X class="h-4 w-4" />
  </Button>
  ...
  <Dialog.Root bind:open={dialogOpen}>
    <Dialog.Content>
      <Dialog.Header>
        <Dialog.Title>{m.group_collaborators_remove_confirm_title()}</Dialog.Title>
        <Dialog.Description>
          {m.group_collaborators_remove_confirm_description({ userName })}
        </Dialog.Description>
      </Dialog.Header>
